...
module.exports = function multimd_table_plugin(md, options) {
  ...
  function table(state, startLine, endLine, silent) {
    ...
    //////////////////////////////////
    // Markdown-it inside the markdown-it module :-)
    var _mdit = markdownit().disable(['table', 'heading', 'lheading', 'html_block']); 
    ...
    _mdit.renderer.rules.heading_open = habrHeading;
    _mdit.renderer.rules.paragraph_open = habrParagraphOpen;
    _mdit.renderer.rules.paragraph_close = habrParagraphClose;    
    ...
    for (r = 0; r < tableToken.meta.tr.length; r++) {
      ...
      for (c = 0; c < trToken.meta.bounds.length - 1; c++) {
        ...
        if (options.multiline && trToken.meta.multiline && trToken.meta.mbounds) {
          ...
          state.md.block.parse(_mdit.render(text.join('\n')), state.md, state.env, state.tokens);
        } else {
          token          = state.push('inline', '', 0);
          token.content  =  _mdit.render(text.trim());
